...
function setFilms(info,date) { // getting information, change and show the schedule
    ...
    info.films.result.forEach((element) => { 
      ...
      filmsHalls[element.film_id].forEach(e => { // showing information
      ...
      Object.keys(sortedHalls).forEach(key => { 
        ...
        sortedHalls[key].forEach( time => {
          ...
          refHall.classList.remove("acceptin-button-disabled"); // active button
          ...
          refHall.onclick = function(){
            sendRequest((hallInfo)=> {
              ...
              window.location.href = "hall.html"; // go to the places page
            },data);
            return false;
          }
...
function getHoursAndMinutes(time) { // duration in hours and minutes
...
function toSecond(time,date) { // conversion in seconds
...
setInterval(()=>{ // relevance of the time 
